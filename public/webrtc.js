...
function connectWebRTC(username) {
    // create and send an offer
    webRTCConnection.createOffer().then(webRTCOffer => {
        socket.send(JSON.stringify({'messageType': 'webRTC-offer', 'recipient': username, 'offer': webRTCOffer}));
        void webRTCConnection.setLocalDescription(webRTCOffer);
    });
    console.log('webrtc offer sent to ' + username)
}
